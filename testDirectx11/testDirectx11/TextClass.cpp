	...
	XMMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence1, "ytk's Engine", 100, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "Hello", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetDebug(char * str, ID3D11DeviceContext *deviceContext)
{
	bool result = UpdateSentence(m_sentence3, str, 20, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
